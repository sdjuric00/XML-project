...
public class ResenjeRepository extends BasicXMLRepository {
    ...
    public Resenje uzmi(final String id) throws XPathException, CannotUnmarshalException {
        ...
        try {
            // get the collection
            System.out.println("[INFO] Retrieving the collection: " + COLLECTION_ID_RESENJE_AUTORSKA_PRAVA_DB);
            col = DatabaseManager.getCollection(connectionProp.uri + COLLECTION_ID_RESENJE_AUTORSKA_PRAVA_DB);
            if (col == null) {
                col = getOrCreateCollection(COLLECTION_ID_RESENJE_AUTORSKA_PRAVA_DB);
            }
            ...
            String xpathExp = "declare variable $data as document-node()* := collection('/" + COLLECTION_ID_RESENJE_AUTORSKA_PRAVA_DB + "');\n" +
                "\n" +
                "for $v in $data\n" +
